...
// 对于一个堆栈，若其入栈序列为1，2，3，...
// ,n，不同的出入栈操作将产生不同的出栈序列。
// 其出栈序列的个数刚好等于结点个数为n的二叉树的个数，且与不同形态的二叉树一一对应。
// 请简要叙述一种从堆栈输入（固定为1，2，3，..., n）和输出序列对应一种二叉树形态
// 的方法，并且以入栈序列1，2，3（n=3）为例确定所有二叉树。
// 答：
// 1）由于二叉树先序遍历序列和中序遍历序列可唯一确定一颗二叉树，因为若入栈序列为1，2，3，...,
// n， 则相当于先序遍历序列是1，2，3，...n,
// 出栈序列就是对应的二叉树的中序遍历序列。有了进栈和出栈序列，
// 相当于知道了一棵二叉树的先序和中序遍历序列，即确定了对应的二叉树。

// 二叉树双序遍历是指：对于二叉树的每一个结点来说，先访问这个结点，再双序遍历它的左子树，然后
// 再一次访问这个结点，接下来双序遍历它的右子树。试写出这种双序遍历的算法。

void Double_order(BTNode *b) {
  if (b != nullptr) {
    visit(b);
    Double_order(b->lchild);
    visit(b);
    Double_order(b->rchild);
  }
}

// 设中序线索二叉树的类型为TBTNode*InThTree，完成以下算法设计：
// 1）设计算法，在一棵中序线索二叉树中寻找结点t的子树上中序下的最后一个结点。
TBTNode *inLast(TBTNode *t) {
  // 沿结点t的右子树链一直走下去，直到遇到其右指针为右线索的结点为止。
  TBTNode *p = t;
  while (p && !p->rtag) // 直到遇到右指针为线索
    p = p->rchild;
  return p;
}
// 2）设计算法，在一棵中序线索二叉树中寻找结点t的中序下的前驱。
TBTNode *inPrior(TBTNode *t) {
  // 如果结点有左线索，则其左线索所指结点即为其中序前驱；若无左线索，则其左子树中中序的最后一个结点即为它的中序前驱。
  TBTNode *p = t->rchild;
  if (p && !t->ltag)
    p = inLast(p);
  return p;
...
TBTNode *treNext(TBTNode *t) {
  TBTNode *p;
  // 如果左线索不存在，则先序遍历下，前序结点为其左孩子
  if (!t->ltag)
    p = t->lchild;
  // 如果左线索存在，右线索不存在，则前序下后继结点为其右孩子
  else if (!t->rtag)
    p = t->rchild;
  else {   // 如果两个线索都存在，没有左孩子，也没有右孩子
    p = t; // 则沿着p的右线索寻找后继节点，直到没有右线索
    while (p && p->rtag)
      p = p->rchild;
    // 此时判断没有右线索时，右孩子是否存在
    if (p)
      // 如果存在，此时右孩子就是结点在前序下的后继结点
      p = p->rchild;
  }
  return p;
}

// 假设二叉树采用二叉链表存储结构，设计一个算法，输出根节点到每个叶子结点的路径。
// 用栈来保存路径上的结点，当p自上至下走时，将所经过的结点依次入栈；当p自下往上走时，
// 将p经过的结点依次出栈；当p来到叶子节点时，自底至顶输出栈中元素就是根到叶子的路径。
int i;
int top = 0;
char pathStack[maxSize];
void allPath(BTNode *p) {
  if (p != nullptr) {
    pathStack[top++] = p->data; // 当所访问结点入栈，即p自上往下走时，结点入栈
    if (p->lchild == nullptr &&
        p->rchild == nullptr) { // 如果当前结点是叶子结点，则打印路径
      for (i = 0; i < top; i++)
        cout << pathStack[i];
    }
    allPath(p->lchild);
    allPath(p->rchild);
    --top; // 此处所访问结点出栈，即p自下往上走，此时结点出栈
  }
}